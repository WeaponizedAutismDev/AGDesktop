		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"ATH0Gram.exe") && exeName != L"ATH0Gram.exe") {
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{A7B0C8D1-47B0-4C2A-8D15-DE5470BA51C0}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
